# Proposed Rust support guide — not the active task contract

Use only after the parent integrates and binds the tested support bytes to a NEW
task/consent. The current frozen support and README are not modified by this proposal.
...
The proposed support reexports `s::{guest, common, ui, settings, kv, clock, host_info, log}`.
...
The proposal uses a checked, address-ordered coalescing free-list over Wasm linear
