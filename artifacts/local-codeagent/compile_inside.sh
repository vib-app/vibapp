...
wit-bindgen rust --world ui-only-reference --out-dir /tmp/project/src /contract/contract.wit
cd /tmp/project
cargo build --release --target wasm32-wasip2 --locked --offline
artifact=/tmp/project/target/wasm32-wasip2/release/vibapp_local_generated_ui.wasm
test -s "$artifact"
test "$(wc -c < "$artifact")" -le 16777216
cp "$artifact" /output/component.wasm
chmod 0600 /output/component.wasm
echo "LOCAL_EXPERIMENTAL_COMPILE_PASS bytes=$(wc -c < "$artifact")"
