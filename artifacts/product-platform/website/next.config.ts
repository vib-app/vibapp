...
const nextConfig: NextConfig = {
  async headers() {
    ...
    if (hostedShell) headers.push({
      ...
      headers: [{
        key: 'Content-Security-Policy',
        value: "default-src 'none'; img-src 'self' data:; style-src 'self'; script-src 'self' 'wasm-unsafe-eval'; connect-src 'self'; worker-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'",
      }],
    });
    return headers;
  },
